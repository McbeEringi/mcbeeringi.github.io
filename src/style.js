...
const _init=()=>{
	...
	header>button{appearance:none;-webkit-appearance:none;flex-shrink:0;display:block;width:48px;height:48px;margin:0;padding:0;border:0;border-radius:50%;background:no-repeat center/70% url(https://icongr.am/feather/menu.svg?color=ffffff);}
...
<header><button></button><nav>
	...
	document.documentElement.addEventListener('touchstart',_=>_,{passive:true});
	document.documentElement.setAttribute('lang',navigator.language.slice(0,2));
};
if(document.readyState=='loading')addEventListener('DOMContentLoaded',_init,{once:true});else _init();
