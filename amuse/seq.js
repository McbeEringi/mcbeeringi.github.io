...
ms2se=()=>{clearTimeout(timer.main2seq);timer.main2seq=setTimeout(()=>{seq.events=main.scores;console.log('scores updated')},300);},
...
tstep=x=>{
	...
	tmp=calced.dat[calced.dat.findIndex(y=>tmp==y.pos)+x];
...
urset=x=>{urstack[2]=[];urstack[0].push(urstack[1]);urstack[1]=x;while(urstack[0].length>Number(localStorage.seq_urMax))urstack[0].shift();console.log(x)},
urdo=x=>{
	let tmp=false;
...
};
{
	...
	scr.addEventListener('wheel',e=>{e.preventDefault();scr.scrollLeft+=(Math.abs(e.deltaX)>Math.abs(e.deltaY)?e.deltaX:e.deltaY)*(e.ctrlKey?.1:1);tmp();});
