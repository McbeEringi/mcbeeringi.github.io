	...
	core=(y,p=0,l=1,ind=[])=>{
		...
		for(let[yi,z]of y.entries()){
			if(typeof z=='object'){
				...
				if(calc&&-pad<pos&&pos<w+pad)calced.dat.push({ind:[...ind,yi],p:p,pos:pos+scr.scrollLeft});
				//if(-pad<pos&&pos<w+pad)ctx.fillText(p.toFixed(2),pos,16,16);ctx.fillText([...ind,yi],pos,32,16);
				pos+=17;
			}
...
};
{
	...
	window.onresize=()=>{c.width=c.parentNode.clientWidth*res;c.height=240*res;ctx.scale(res,res);a2c(1);};
